...
//*TODO : Please be make sure if you have this base + A fragment + B fragment > and you call this from B, you get= activity null because you are destroying intance B itself.
    ...
    fun any_preorder_closedRestaurant(is_restaurant_closed: Boolean?, pre_order: Boolean?, msg: String?): Boolean {
        ...
        if ((is_restaurant_closed != null && is_restaurant_closed == true) && (pre_order != null && pre_order == false)
                ||
                (is_restaurant_closed != null && is_restaurant_closed == true) && pre_order == null
        ) {
    ...
    fun getrestaurantstatus(is_restaurant_closed: Boolean?, pre_order: Boolean?): RestaurantState {

        val is_restaurant_closed_ = is_restaurant_closed ?: false
        val pre_order_ = pre_order ?: false

        if (is_restaurant_closed_ && pre_order_) {
            // (true and true ) Pre rest
            val state = RestaurantState.PRE
            return state
        } else if (is_restaurant_closed_ && !pre_order_) {
            // (true and false ) close rest
            val state = RestaurantState.CLOSED
            return state

        } else {
            // open all this condition
            val state = RestaurantState.OPEN
            return state
        }

    }

    fun getdeliverymsg_error(jsonObject: JsonObject): String {
